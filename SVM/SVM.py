...
运行结果：
    ...
    运行时长：50s
---------------------------------
code from https://github.com/Dod-o/Statistical-Learning-Method_Code/blob/master/SVM/SVM.py
因为SMO参考pkudodo大佬的代码写的pytorch版经常涉及到tensor的大小比较，这又涉及到tensor的值是否为空的问题
The error RuntimeError: Boolean value of Tensor with no values is ambiguous typically occurs when you try to evaluate a tensor in a boolean context,
such as an if statement, but the tensor is empty. This can happen if the tensor has no elements.
所以直接使用了pkudodo的代码了，在此感谢
'''
...
if __name__ == '__main__':
    ...
    trainDataList, trainLabelList = loadData("/root/Desktop/course/datasets/Mnist/mnist_train.csv")

    # 获取测试集及标签
    print('start read testSet')
    testDataList, testLabelList = loadData("/root/Desktop/course/datasets/Mnist/mnist_test.csv")
